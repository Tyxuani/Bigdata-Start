package com.galaxy.metrics.historyrams;

import com.codahale.metrics.ConsoleReporter;
import com.codahale.metrics.Histogram;
import com.codahale.metrics.MetricRegistry;
import com.codahale.metrics.UniformReservoir;

import java.util.concurrent.ThreadLocalRandom;
import java.util.concurrent.TimeUnit;

import static com.galaxy.metrics.common.CommonUtil.costTime;

public class UniformReservoirsExample {
    private static MetricRegistry registry = new MetricRegistry();
    private static ConsoleReporter reporter = ConsoleReporter.forRegistry(registry)
            .convertRatesTo(TimeUnit.SECONDS)
            .convertDurationsTo(TimeUnit.SECONDS)
            .build();

    //默认统计最近1028次的median, Reservoir存储池抽样是针对median的统计
    private static Histogram histogram = new Histogram(new UniformReservoir());

    public static void main(String[] args) {

        reporter.start(5, TimeUnit.SECONDS);
        registry.register("UniformReservoirs-Example", histogram);

        while (true) {
            doSearch();
            costTime();
        }
    }

    private static void doSearch() {

        //搜索过程忽略
        //直方图统计搜索量(此处给的随机数)
        histogram.update(ThreadLocalRandom.current().nextInt(10));
    }


}
